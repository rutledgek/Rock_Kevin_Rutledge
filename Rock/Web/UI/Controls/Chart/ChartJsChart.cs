...
{
    /// <summary>
    /// Provides the foundation for a ChartJS implementation of a Rock chart control.
    /// </summary>
    public abstract class ChartJsChart : CompositeControl, IRockChart
    {
        ...
        /// <param name="defaultSeriesName"></param>
        public void SetChartDataItems( IEnumerable<IChartData> chartDataItems, string defaultSeriesName = null )
        {
...
" );
        ...
        private void SetLabelStyle( Label label, ChartTextElementStyle format )
        {
            ...
            {
                label.ForeColor = ColorTranslator.FromHtml( format.ForeColor );
            }

            if ( !string.IsNullOrWhiteSpace( format.FontName ) )
            {
                label.Font.Name = format.FontName;
            }

            if ( format.FontSize != null )
            {
                label.Font.Size = new FontUnit( format.FontSize.Value );
            }
        ...
        // Gets a DateTime from a JavaScript datetime stamp,
        // defined as the number of milliseconds elapsed since 1/1/1970 00:00:00 UTC.
