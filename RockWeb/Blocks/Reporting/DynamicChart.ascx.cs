...
<code>
    ...
    public partial class DynamicChart : RockBlock
    {
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            pcPieChart.ShowSegmentLabels = GetAttributeValue( "PieShowLabels" ).AsBooleanOrNull() ?? false;
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( _chartType == "pie" )
            {
                ...
                // MetricTitle/SeriesName/SeriesId (string), Value/YValue/YValueTotal (numeric).
                // It can only be used to plot category data, not a time series.
                if ( categoryFieldName == null && seriesFieldName != null )
                {
        ...
        {
            var availableFieldNames = sampleRow.Table.Columns.Cast<DataColumn>()
                .Select( dc => dc.ColumnName )
                .ToList();
            foreach ( var fieldName in fieldNames )
            {
                if ( availableFieldNames.Contains( fieldName, StringComparer.OrdinalIgnoreCase ) )
                {
                    return fieldName;
                }
            }
